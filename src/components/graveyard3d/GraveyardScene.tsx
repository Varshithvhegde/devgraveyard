  ...
  return (
    <>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.002, 0]}>
        ...
        <meshStandardMaterial color="#2a2415" roughness={1} transparent opacity={0.5} />
      </mesh>
    </>
  );
}
...
function GrassTufts() {
  ...
  if (!tufts.current.length) {
    for (let i = 0; i < 180; i++) {
      tufts.current.push({ x: (Math.random()-0.5)*70, z: (Math.random()-0.5)*70, s: 0.2+Math.random()*0.5, r: Math.random()*Math.PI });
    }
  }
  return (
    <group>
      {tufts.current.map((t, i) => (
        <mesh key={i} position={[t.x, 0.05, t.z]} rotation={[Math.PI/2, t.r, 0]}>
          <planeGeometry args={[t.s, t.s*1.6]} />
          <meshBasicMaterial color="#1e2a0a" side={THREE.DoubleSide} transparent opacity={0.55} />
        </mesh>
      ))}
    </group>
  );
...
function BareTree({ pos }: { pos: [number,number,number] }) {
  const branches: [number,number,number,number,number,number,number][] = [
    [0.5,3.8,0,0,0,-0.5,1.4], [-0.6,3.5,0,0,0,0.55,1.3],
    [0.3,4.4,0.3,-0.3,0,-0.25,1.2], [-0.35,4.1,-0.25,0.25,0,0.3,1.1], [0.1,5.0,0,0,0,-0.15,0.9],
  ];
  return (
    <group position={pos}>
...
// ─── Tombstone extruded shape ─────────────────────────────────────────────────
function makeTombShape() {
  const w=0.34, h=0.95, r=w;
  const s = new THREE.Shape();
  s.moveTo(-w,0); s.lineTo(-w,h);
  s.absarc(0,h,r,Math.PI,0,false);
...
function FlickerCandle({ x, phase=0 }: { x: number; phase?: number }) {
  ...
  useFrame(({ clock }) => {
    ...
    const flicker = Math.sin(t*8+phase)*0.08 + Math.sin(t*13+phase*2)*0.04 + 1;
    if (flameRef.current) {
      flameRef.current.scale.set(flicker*0.8, flicker, flicker*0.8);
      flameRef.current.position.y = 0.11 + Math.sin(t*6+phase)*0.008;
    }
    if (lightRef.current) {
      lightRef.current.intensity = (1.5 + Math.sin(t*7+phase)*0.4) * 1.2;
    }
  });
  return (
    <group position={[x, 0.2, 0.14]}>
      ...
      <pointLight ref={lightRef} color="#fbbf24" intensity={1.5} distance={2.5} position={[0,0.12,0]} />
    </group>
  );
}

// ─── Soul wisps (for tombstones with candles lit) ─────────────────────────────
function SoulWisps({ count }: { count: number }) {
  const n = Math.min(count, 5);
  const refs = useRef<THREE.Mesh[]>([]);
  const phases = useRef(Array.from({length:n},()=>Math.random()*Math.PI*2));
  useFrame(({ clock }) => {
    const t = clock.getElapsedTime();
    refs.current.forEach((m, i) => {
      ...
      mat.opacity = life < 0.2 ? life/0.2 * 0.5 : (1-life)*0.5;
    });
  });
  return (
    <group>
      {Array.from({length:n}).map((_,i)=>(
        <mesh key={i} ref={el=>{ if(el) refs.current[i]=el; }} position={[0,1.2,0.08]}>
          <sphereGeometry args={[0.025,6,6]} />
          <meshBasicMaterial color="#e8d8ff" transparent opacity={0.4} depthWrite={false} />
        </mesh>
      ))}
    </group>
  );
...
// ─── Candle burst particles ───────────────────────────────────────────────────
function CandleBurst({ active }: { active: boolean }) {
  ...
  useFrame(({ clock }) => {
    if (active && !started.current) { t0.current = clock.getElapsedTime(); started.current = true; }
    if (!started.current || !particles.current) return;
    const elapsed = Math.min(clock.getElapsedTime()-t0.current, 1.2);
    const arr = particles.current.geometry.attributes.position.array as Float32Array;
    for (let i=0; i<COUNT; i++) {
      const d = dirs.current[i];
      arr[i*3]   = d.x * elapsed * 0.7;
      arr[i*3+1] = d.y * elapsed * 0.7 - elapsed*elapsed*0.4;
      arr[i*3+2] = d.z * elapsed * 0.7;
    }
    particles.current.geometry.attributes.position.needsUpdate = true;
    (particles.current.material as THREE.PointsMaterial).opacity = Math.max(0, 1-elapsed);
    if (elapsed >= 1.2) started.current = false;
  });

  return (
    <points ref={particles} geometry={geo} position={[0,0.5,0.1]}>
      <pointsMaterial color="#fbbf24" size={0.06} transparent opacity={1} sizeAttenuation depthWrite={false} />
    </points>
  );
}
...
function Tombstone3D({
  ...
  // Entrance: rise from underground
  const enterProgress = useRef(0);
  const ENTER_DELAY = index * 0.12;
  ...
  useFrame(({ clock }, delta) => {
    ...
    groupRef.current.userData.h = h + (ht - h) * Math.min(1, delta * 7);
    groupRef.current.position.y = groupRef.current.userData.h * 0.2;
    groupRef.current.rotation.x = groupRef.current.userData.h * -0.04;

    // Idle sway
    if (!isHovered) {
      groupRef.current.rotation.z = Math.sin(clock.getElapsedTime()*0.3 + index)*0.004;
    }

    if (glowRef.current) {
      glowRef.current.intensity = (groupRef.current.userData.h ?? 0) * 2.0 + (hasCandle ? 0.8 : 0);
    }
  });
  ...
  return (
    ...
    >
      ...
      <mesh castShadow receiveShadow position={[0,0.18,0]} rotation={[Math.PI/2,0,0]}>
        <extrudeGeometry args={[makeTombShape(),{depth:0.22,bevelEnabled:true,bevelThickness:0.018,bevelSize:0.014,bevelSegments:4,curveSegments:24}]} />
      ...
      <mesh position={[0,0.78,0.135]}>
        <shapeGeometry args={[(() => {
          const s=new THREE.Shape();
          s.moveTo(-0.22,0); s.lineTo(-0.22,0.65); s.lineTo(0.22,0.65); s.lineTo(0.22,0); s.lineTo(-0.22,0);
          return s;
        })()]} />
      ...
      {/* Text */}
      <Text position={[0,1.32,0.14]} fontSize={0.11} color={h?"#ffffff":"#ede8e0"}
      ...
      <Text position={[0,1.02,0.14]} fontSize={0.065} color={h?"#d8d0c8":"#a8a098"}
        anchorX="center" anchorY="middle" outlineWidth={0.006} outlineColor="#0a0908" outlineOpacity={1}>
        {`${new Date(tombstone.born_at).getFullYear()} — ${new Date(tombstone.died_at).getFullYear()}`}
      </Text>
      <Text position={[0,0.76,0.14]} fontSize={0.054} color={h?"#e0c8ff":"#a880c8"}
        anchorX="center" anchorY="middle" maxWidth={0.46} textAlign="center"
        outlineWidth={0.006} outlineColor="#0a0908" outlineOpacity={1}>
        {tombstone.cause_of_death.length>18 ? tombstone.cause_of_death.slice(0,17)+"…" : tombstone.cause_of_death}
      </Text>

      {/* Candles — always lit if user has candle, or on hover */}
      {(hasCandle || isHovered) && (
        <>
...
function Fireflies() {
  ...
  const base = useRef(Float32Array.from({length:COUNT*3},(_,i)=>
    i%3===1 ? 0.5+Math.random()*2.5 : (Math.random()-0.5)*34));
  const phases = useRef(Array.from({length:COUNT},()=>Math.random()*Math.PI*2));
  const geo = new THREE.BufferGeometry();
  geo.setAttribute("position", new THREE.BufferAttribute(base.current.slice(),3));
  ...
  useFrame(({clock})=>{
    ...
    const t = clock.getElapsedTime();
    for (let i=0;i<COUNT;i++) {
      arr[i*3+1] = base.current[i*3+1] + Math.sin(t*0.7+phases.current[i])*0.4;
      arr[i*3]   = base.current[i*3]   + Math.cos(t*0.4+phases.current[i])*0.25;
    }
...
function Scene({
  ...
  return (
    <>
      <ambientLight intensity={0.42} color="#8090c0" />
      <directionalLight position={[-10,18,6]} intensity={1.6} color="#d0e0ff" castShadow
  ...
  const toggleVote = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.success("Resurrection vote cast ✨");
      }
        ...
        <div className="p-7">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <button onClick={toggleCandle} disabled={candleLoading}
              ...
              <span className="text-2xl" style={{filter:isLit?"drop-shadow(0 0 8px #fbbf24)":"none"}}>
                🕯️
              </span>
              <span className={`font-mono text-sm font-bold ${isLit?"text-amber-400":"text-zinc-500"}`}>
                {candleCount}
              </span>
            ...
            <button onClick={toggleVote} disabled={voteLoading}
              ...
              <span className="text-2xl" style={{filter:isVoted?"drop-shadow(0 0 8px #34d399)":"none"}}>
                ✨
              </span>
              <span className={`font-mono text-sm font-bold ${isVoted?"text-emerald-400":"text-zinc-500"}`}>
                {voteCount}
              </span>
            ...
            <div className="h-1 rounded-full overflow-hidden" style={{background:"rgba(255,255,255,0.06)"}}>
              <div className="h-full rounded-full transition-all duration-700"
                style={{
                  ...
                  boxShadow:isVoted?"0 0 6px rgba(52,211,153,0.6)":"none",
                }} />
            </div>
...
export default function GraveyardScene({ tombstones }: { tombstones: TombstoneWithStats[] }) {
  ...
  // Load current user's candles + votes on mount
  useEffect(()=>{
  ...
  return (
    <div className="relative w-full h-full">
      <Canvas camera={{position:[0,7,16],fov:52,near:0.1,far:150}} shadows
